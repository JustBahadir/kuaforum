...
export function ShopGalleryCard({
    ...
    try {
      ...
      if (queryClient) {
        queryClient.invalidateQueries({
          queryKey: [`shop-${dukkanId}-gallery`]
        });
      }
  ...
  return <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        {isAdmin && <Button variant="outline" size="sm" onClick={() => document.getElementById('gallery-upload')?.click()}>
            <Plus className="h-4 w-4 mr-2" />
            Resim Ekle
            <input id="gallery-upload" type="file" className="hidden" onChange={handleFileChange} accept="image/jpeg,image/png,image/gif" disabled={isUploading} />
          </Button>}
      </CardHeader>
      <CardContent className="py-[88px]">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {images.map((image, index) => <div key={index} className="relative group aspect-square">
              <img src={image} alt={`İşletme Resmi ${index + 1}`} className="w-full h-full object-cover rounded-md cursor-pointer" onClick={() => setSelectedImage(image)} />
              {isAdmin && <button className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => removeImage(image)}>
                  <X className="h-4 w-4" />
                </button>}
            </div>)}
        ...
        {/* Image Modal */}
        {selectedImage && <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="max-w-4xl max-h-[90vh] relative">
              <img src={selectedImage} alt="İşletme Resmi" className="max-w-full max-h-[90vh] object-contain" />
              <button className="absolute top-4 right-4 bg-red-500 text-white rounded-full p-2" onClick={() => setSelectedImage(null)}>
                <X className="h-6 w-6" />
              </button>
            </div>
          </div>}

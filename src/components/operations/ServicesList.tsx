    ...
    <div className="p-4 border border-gray-200 rounded-md">
      {islemler.length > 0 ? (
        <div className="space-y-2">
          {islemler.map((hizmet) => (
            <div 
              key={hizmet.id || hizmet.kimlik} 
              className={`p-3 border rounded-md ${
                selectedItems.includes(hizmet.id as string) || 
                selectedItems.includes(hizmet.kimlik as string) || 
                selectedItems.includes(hizmet.id as number) 

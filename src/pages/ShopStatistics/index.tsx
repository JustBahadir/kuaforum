...
export default function ShopStatistics() {
  ...
  const [currentView, setCurrentView] = useState<"daily" | "weekly" | "monthly" | "yearly">("monthly");
  
  // Metrics state for MetricsCards
  ...
  // Update metrics data for the MetricsCards component
  useEffect(() => {
    // Set metrics based on the filtered operations
    setMetricsData({
      totalRevenue: stats.totalRevenue,
      totalAppointments: stats.totalOperations,
      averageTicket: stats.averageTicket,
      customerCount: new Set(filteredOperations.map(op => op.musteri_id).filter(Boolean)).size,
      completionRate: 100 // Default to 100% for now
    });
  }, [stats, filteredOperations]);

  // Handle tab change
  const handleViewChange = (value: string) => {
    setCurrentView(value as "daily" | "weekly" | "monthly" | "yearly");
  };
        ...
        <Card className="mb-6">
          <CardHeader className="pb-2">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <CardTitle>Zaman Görünümü</CardTitle>

...
const HistoryTab = ({
  ...
  const handleAddWorkplace = () => {
    ...
    const newEntry = { isYeri: newIsYeri.trim(), gorevPozisyon: newGorevPozisyon.trim() };
  ...
  const handleSaveWorkplace = () => {
    ...
    updated[workplaceEditIndex] = {...workplaceEditValues};
    setWorkplaceList(updated);
    setWorkplaceEditIndex(null);
    updateHistoryField(updated, documentList, competitionList, cvText);
  };
    ...
    return (
      <tr key={index} className="border-b hover:bg-gray-50">
        <td className="p-2">
          {isEditing ? (
            <input
              className="bg-white text-black rounded-md border border-gray-300 px-2 py-1 w-full placeholder:text-gray-400"
            ...
            <span className="text-black">{item.isYeri}</span>
          )}
        </td>
        <td className="p-2">
          {isEditing ? (
            <input
              ...
              placeholder="Görev veya pozisyon giriniz..."
              disabled={isLoading}
            />
          ) : (
            <span className="text-black">{item.gorevPozisyon}</span>
          )}
        </td>
  ...
  const renderDocumentRow = (doc: string, index: number) => {
    ...
    return (
      <tr key={index} className="border-b hover:bg-gray-50">
        ...
        <td className="p-2 flex gap-2 justify-end">
          {isEditing ? (
            <Button size="sm" variant="default" onClick={handleSaveDocument} title="Kaydet" className="bg-purple-600 text-white hover:bg-purple-700">
              <Save size={16} />
            </Button>
          ) : (
            <>
      ...
      <tr key={index} className="border-b hover:bg-gray-50">
        <td className="p-2">
          {isEditing ? (
            ...
            <span className="text-black">{comp}</span>
          )}
        </td>
        <td className="p-2 flex gap-2 justify-end">
          {isEditing ? (
            <Button size="sm" variant="default" onClick={handleSaveCompetition} title="Kaydet" className="bg-purple-600 text-white hover:bg-purple-700">
              <Save size={16} />
            </Button>
          ) : (
            <>
    ...
    <div className="space-y-6 max-w-4xl mx-auto bg-white p-6 rounded-md shadow-sm border border-gray-200">
      ...
      <div>
        ...
        {workplaceList.length > 0 ? (
          ...
          <p className="text-sm text-gray-500">Henüz iş tecrübesi eklenmedi.</p>
        )}
      </div>

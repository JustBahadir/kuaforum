...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell, Line, Legend, TooltipProps } from "recharts";
...
function generateInsights(operations: Operation[], personnel: {
  ...
  insights.push(`En yüksek ciroyu ${maxRevenuePersonelName} elde etti (${formatCurrency(revenueByPersonnel[maxRevenuePersonelId || ""] || 0)}).`);
...
export function PersonnelPerformanceReports({
  ...
  return <div className="space-y-6">
      ...
      <DateControlBar dateRange={dateRange} onDateRangeChange={setDateRange} />
      ...
      <Card>
        ...
        overflowX: "auto"
          ...
          <div style={{
          minWidth: `${dailyDataMap.length * 70}px`,
          height: 300
        }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dailyDataMap} margin={{
              top: 20,
              right: 20,
              left: 20,
              bottom: 60
                ...
                <XAxis dataKey="date" angle={-45} textAnchor="end" height={70} tick={{
                fontSize: 12
              }} />
                <YAxis yAxisId="left" tickFormatter={v => `₺${v}`} domain={[0, 'auto']} label={{
                value: 'Ciro (₺)',
                angle: -90,
                position: 'insideLeft'
              }} />
                <YAxis yAxisId="right" orientation="right" domain={[0, 'auto']} label={{
                value: 'İşlem Sayısı',
                angle: 90,
                position: 'insideRight'
              }} />
                <Tooltip formatter={customTooltipFormatter} labelFormatter={label => `Tarih: ${label}`} />
                <Legend />
                <Bar yAxisId="left" dataKey="revenue" name="Ciro" fill="#8b5cf6" radius={[4, 4, 0, 0]} barSize={30} />
                <Line yAxisId="right" type="monotone" dataKey="operations" name="İşlem Sayısı" stroke="#a78bfa" strokeWidth={3} dot={{
                r: 4
              }} activeDot={{
                r: 6
              }} />
      ...
      <Card className="md:flex md:flex-row md:gap-6 p-4">
        <div className="md:w-1/2 h-72">
          <PieChart width={380} height={280}>
            <Pie data={serviceDataMap} dataKey="revenue" nameKey="name" cx="50%" cy="50%" outerRadius={100} fill="#8b5cf6" label={({
            name,
            percent
          }: {
            name: string;
        ...
        <div className="md:w-1/2 flex flex-col justify-center p-4">
          ...
          <ul className="space-y-2">
            ...
            return <li key={idx} className="flex items-center gap-4 text-sm cursor-default select-none">
                  <span className="inline-block w-4 h-4" style={{
                backgroundColor: ["#a78bfa", "#c4b5fd", "#d8b4fe", "#e9d5ff", "#f3e8ff"][idx % 5]
              }} />
          ...
          <table className="min-w-full divide-y divide-gray-200 border">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {tableData.length === 0 ? <tr>
                ...
                </tr> : tableData.map(op => <tr key={op.id} className="hover:bg-gray-50">
